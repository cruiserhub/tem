...
class MemoryNet(nn.Module):
    ...
    def __init__(self, n_iteration = 50, kappa = 0.8, lamda = 0.9, yita = 0.1, nx_in = 26, ng_in = 17, nx_out = 20, ng_out = 10, num_class = 7, hop_type = 1, heb_type = 0):
        ...
        self.register_buffer('M2', torch.zeros(1, self.dim_g * self.dim_x, self.dim_g * self.dim_x))
        self.register_buffer('M3', torch.zeros(1, self.dim_g * self.dim_x, self.dim_g * self.dim_x))
        ...
        self.predict_sensory2 = MLP(in_dim=nx_out, out_dim=num_class, bias=[True], num_layer=1)
        self.predict_sensory3 = MLP(in_dim=nx_out, out_dim=num_class, bias=[True], num_layer=1)
        self.reliability = MLP(in_dim=nx_out * ng_out, out_dim=2, bias=[True], num_layer=1)
   ...
   # checked with no problem, key variables, p_, M
    def retrive_memory(self, query, M, n_iteration = None):
        """
        Generator: pt ~ N(.| u = attractor(gt_, Mt-1), sigma = f(u))
...
# hebbain learning
    ...
    def clear_memory(self):
        ...
        self.M2 = torch.zeros(1, self.dim_g * self.dim_x, self.dim_g * self.dim_x)
        self.M3 = torch.zeros(1, self.dim_g * self.dim_x, self.dim_g * self.dim_x)
        self.x_buffer = []
        self.g_buffer = []
        self.label_buffer = []

    def forward(self, x, g, M, heb = True, n_iteration = None, event_type = None):
        if self.x_transform:
        ...
        p =  self.leaky_relu(torch.clamp(p, min=-1, max=1))
        #
        p_ = self.retrive_memory(torch.matmul(g_, self.W_tile), M, n_iteration=n_iteration)
        ...
        if heb:
            if event_type == None:
                self.M = self.form_memory(p[0], p_[0], M)
            elif event_type == 0:
                self.M2 = self.form_memory(p[0], p_[0], M)
            elif event_type == 1:
                self.M3 = self.form_memory(p[0], p_[0], M)
        x_out = torch.matmul(self.W_repeat, p_.squeeze())

        # g_out = torch.matmul(self.W_tile, p_.squeeze())
        if event_type == None:
            x_inf = self.predict_sensory(x_out)
        elif event_type == 0:
            x_inf = self.predict_sensory2(x_out)
        elif event_type == 1:
            x_inf = self.predict_sensory3(x_out)
...
class MemoryGame(MemoryNet):
    ...
    def placefield(self):
        field = np.zeros((2, self.size))
        for k in range(2):
            for i in range(field.shape[1]):
                # distance generation
                field[k, i] = (i - self.agent.pos[k]-2) ** 2
                # gaussian density, but before exponential to help learning identity mapping input to output
        field = torch.exp(- 0.1 * torch.from_numpy(field).resize(2 * (self.size)).float())
        return field

    def data_collect(self, chunks = 3):
        Pos = zip([np.random.randint(2, self.size+2) for i in range(5)], [np.random.randint(2, self.size+2) for i in range(8)])
        ...
        for init_pos in Pos:
            ...
            for act in actions:
                # print(act, agent.pos)
                self.agent.act(act)
                pos =torch.cat([torch.eye(self.size)[self.agent.pos[0]-2], torch.eye(self.size)[self.agent.pos[1]-2]])
                self.x_buffer.append(
                    torch.from_numpy(self.env.sensory[self.agent.pos].ravel()).view(1, -1).type(torch.FloatTensor)-1)
                # print(self.env.objects[agent.pos])
                self.g_buffer.append(pos.view(1, -1))
    ...
    def train_g(self, Optim, xs, gs, labels, heb = True):
        Loss, Loss_heb = [], []
        # replay
        ...
        self.M2 = torch.autograd.Variable(self.M2)
        self.M3 = torch.autograd.Variable(self.M3)
        ...
        for i, (x, g, label) in enumerate(zip(xs, gs, labels)):
            ...
            x_inf, g_, x_ , p, p_= self.forward(x, g, self.M, heb=heb, event_type=None)
            if torch.argmax(x_inf) in [1, 5]:
                x_inf, g_, x_, p, p_ = self.forward(x, g, self.M2, heb=heb, event_type=0)
            elif torch.argmax(x_inf) in [2, 4]:
                x_inf, g_, x_, p, p_ = self.forward(x, g, self.M3, heb=heb, event_type=1)
    ...
    def test_g(self):
        ...
        for x, g, label in zip(xs, gs, labels):
            x_inf, g_, x_, _, _ = self.forward(x, g, self.M, heb = False, n_iteration=10, event_type=None)
            # print ('g', self.transform_g(g))
            # print (x_)
            predict = torch.argmax(x_inf)
            print('first run', predict, label)
            if predict in [1, 5]:
                x_inf, g_, x_, _, _ = self.forward(x, g, self.M2, heb=False, n_iteration=10, event_type=0)
                predict = torch.argmax(x_inf)
                print ('second run', predict, label)
            elif predict in [2, 4]:
                x_inf, g_, x_, _, _ = self.forward(x, g, self.M3, heb=False, n_iteration=10, event_type=1)
                predict = torch.argmax(x_inf)
                print ('second run', predict, label)
            # print (predict, label)
...
if __name__ == '__main__':
    ...
    size =10
    game = MemoryGame(size = size, n_iteration=5, kappa = 0.5, lamda = 0.99, yita = 0.5, hop_type=1, heb_type = 0, nx_in = 26, ng_in = 2 * size, nx_out = 20, ng_out = 15, num_class=8)
    Optim = torch.optim.Adam(game.parameters(), lr=1e-3)
    Acc, k = 0, 0
    ...
    for i in range(int(1e4)):
        ...
        indice = torch.tensor(np.random.randint(len(game.x_buffer), size = 100))
        # print ('len', len(game.x_buffer), indice)
        xs = torch.stack(game.x_buffer)[indice]
        gs = torch.stack(game.g_buffer)[indice]
        labels = torch.stack(game.label_buffer)[indice]
        loss, _ = game.train_g(Optim, xs, gs, labels)
        Loss += loss
        ...
        if (i+1)%10 == 0 and i>0:
            ...
            Acc += acc
            k += 1
            print('accumulated accuracy', Acc/k)
