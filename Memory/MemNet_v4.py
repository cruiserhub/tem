...
class MemoryNet(nn.Module):
    ...
    def __init__(self, n_iteration = 50, kappa = 0.8, lamda = 0.9, yita = 0.1, nx_in = 26, ng_in = 17, nx_out = 20, ng_out = 10, num_class = 7, hop_type = 1, heb_type = 0):
        ...
        self.lam_net = MLP(in_dim=nx_out * ng_out, out_dim=1, num_layer=1)
        self.yita_net = MLP(in_dim=nx_out * ng_out, out_dim=1, num_layer=1)
...
# hebbain learning
    def form_memory(self, p, p_):
        ...
        if self.heb_type == 0:
            ...
            # print ('remember', F.sigmoid(self.lam_net(p - p_)).shape)
            self.M = F.sigmoid(self.lam_net(p - p_)) * self.M + F.sigmoid(self.yita_net(p - p_)) * M_new
    ...
    def forward(self, x, g, M, heb = True, n_iteration = None):
        if self.x_transform:
...
class MemoryGame(MemoryNet):
    ...
    def test_g(self):
        ...
        for x, g, label in zip(xs, gs, labels):
            x_inf, g_, x_, _, _ = self.forward(x, g, self.M, heb = False, n_iteration=10)
...
if __name__ == '__main__':
    ...
    size =10
    game = MemoryGame(size = size, n_iteration=5, kappa = 0.5, lamda = 0.99, yita = 0.5, hop_type=1, heb_type = 1, nx_in = 26, ng_in = 2 * size, nx_out = 20, ng_out = 15, num_class=8)
