...
class MemoryGame(MemoryNet):
    ...
    def train_g(self, heb = True, shuffle = False):
        ...
        for i, (x, g, label) in enumerate(zip(self.x, self.g, self.labels)):
            ...
            x_inf, g_, x_ , p, p_= self.forward(x, g, heb=heb)
            ...
            loss = entropy(x_inf.view(self.batch_size, -1), torch.LongTensor(label).view(self.batch_size)) #+ 1e-2 * loss_reg
    ...
    def test_g(self, show = False):
        ...
        for x, g, label in zip(self.x, self.g, self.labels):
            x_inf, g_, x_, _, _ = self.forward(x, g, heb = False)
...
if __name__ == '__main__':
    ...
    for i in range(int(1e4)):
        ...
        if i%20 == 0:
            print ('loss', 'loss_heb', Loss, Loss_heb)
