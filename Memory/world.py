...
class Envs(Grid):
    def __init__(self, grid_size = 15, type = 'default'):
        ...
        if type == 'default':
            ...
            set_gross = [0, 3, 0, 1, 0, 3, 2]
            self.objects_set_gross = {w:set_gross[i] for i,w in enumerate(set(self.objects_type))}
            self.objects_set = {w:i+1 for i,w in enumerate(set(self.objects_type))}
            self.objects_nhot = [self.encode_nhot(w) for w in self.objects_type]
            objects, pos_vect = [], []
            for i in range(100):
                objects.append(self.objects_type[np.random.randint(len(self.objects_type))])
                pos_vect.append((np.random.randint(2, self.grid_size_y + 2*VISIBLE_RADIUS), np.random.randint(2, self.grid_size_x + 2*VISIBLE_RADIUS)))
            ...
            self.objects_gross = np.zeros((self.grid_size_y + 2 * VISIBLE_RADIUS, self.grid_size_x + 2 * VISIBLE_RADIUS, 1))
            for i, pos in enumerate(pos_vect):
                ...
                self.objects_gross[pos[0] - 1: pos[0] + 2, pos[1] - 1: pos[1] + 2] = self.objects_set_gross[objects[i]]
...
class Agent(Envs):
    ...
    def wall(self, pos):
        ...
        if np.sum(self.grid[pos]) < 0:
            ...
            #                 self.grid[pos] >= 0]
            pos = (np.random.randint(self.grid_size_y) + 2*VISIBLE_RADIUS, np.random.randint(self.grid_size_x) + 2*VISIBLE_RADIUS)
            return pos
        else:
            return pos
