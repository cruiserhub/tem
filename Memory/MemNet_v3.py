...
class MemoryNet(nn.Module):
    ...
    def __init__(self, n_iteration = 50, kappa = 0.8, lamda = 0.9, yita = 0.1, nx_in = 26, ng_in = 17, nx_out = 20, ng_out = 10, num_class = 7, hop_type = 1, heb_type = 0):
        ...
        self.ln1 = nn.LayerNorm(ng_out, elementwise_affine=False, eps=1e-03)
        ...
        self.ln2 = nn.LayerNorm(nx_out, elementwise_affine=False, eps=1e-03)
...
class MemoryGame(MemoryNet):
    ...
    def data_collect(self, chunks = 3):
        length = 8
        Pos = zip([np.random.randint(2, self.size+2) for i in range(length)], [np.random.randint(2, self.size+2) for i in range(length)])
    ...
    def train_g(self, Optim, xs, gs, labels, heb = True, shuffle = False):
    ...
    def test_g(self, plot = False):
        ...
        if plot:
            ...
            ax2 = ax1.twinx()
            ax1.hist(torch.stack(X_).data.numpy().ravel())
            ax2.hist(torch.stack(G_).data.numpy().ravel(), alpha=0.5, color='r')
            ax2.hist(torch.stack(X_inf).data.numpy().ravel(), alpha=0.5, color='g')
